...
main(int argc, char *argv[]){
  ...
  if(MatC.rows/block<1){
    printf("Block too large Block Size %d Rows %d\n Exiting\n",block,MatC.rows);
    exit(EXIT_FAILURE);
  }
  int blockalloc=block; // if the rows%block is larger
  if (MatC.rows%block!=0 ) blockalloc++;
  /* Allocate an arrays that will contain the  blocks COO and CSC  */
  struct cooMatrix *MatrixCOOArrA =  malloc(blockalloc*blockalloc*sizeof(struct cooMatrix));
  struct cooMatrix *MatrixCOOArrB = malloc(blockalloc*blockalloc*sizeof(struct cooMatrix));
  ...
  for(int j=0;j<MatA.rows;j++){
    for(int r=MatA.csc_c[j];r<MatA.csc_c[j+1];r++){
      ...
      MatrixCOOArrA[ptr].coo_r = (int *)realloc(MatrixCOOArrA[ptr].coo_r,MatrixCOOArrA[ptr].nnz*sizeof(int));
      MatrixCOOArrA[ptr].coo_c = (int *)realloc(MatrixCOOArrA[ptr].coo_c,MatrixCOOArrA[ptr].nnz*sizeof(int));
  ...
  struct cscMatrix *MatrixArrB = malloc(blockalloc*blockalloc*sizeof(struct cscMatrix));
