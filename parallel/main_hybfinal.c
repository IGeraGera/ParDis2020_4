/* This is implementaion uses MPI+openMP without dissecting the matrix. The workload for MPI is assigned statically.
...
checkArgsNum(int argc){
	switch(argc){
		case 1:
			...
			exit(EXIT_FAILURE);
		case 2:
			fprintf(stderr, "Line:%d One arguments are missing\n",__LINE__);
			exit(EXIT_FAILURE);
		case 3:
			break;
		default:
