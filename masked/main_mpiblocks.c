/* MPI only implementation with matrix dissection. The matrixes are dissected and the A matrix is broadcasted to every process. */
...
main(int argc, char *argv[]){
	...
	for (int Ccol=workStart;Ccol<workEnd;Ccol++){
		...
		for (int Crow=0;Crow<blockalloc;Crow++){
			...
			int firstFlag = 1;
			/* if (rank==1) printf("Arri %d Arrj %d rows %d\n",Crow,Ccol,MatrixArrA[Cptr].rows); */
